...
bool DependencyFinder::addDependency(const string &aObjectName,
  ...
  // Check if the object is already in the mapping of object name to matrix
  // index.
  ...
  ObjectIndexMap::iterator dependencyLocation =
      mObjectIndices.find(aDependency);
  ...
  H_ASSERT(mDependencyMatrix.size() > objectLocation->second,
           "addDependency failure");
  H_ASSERT(mDependencyMatrix[objectLocation->second].size() >
               dependencyLocation->second,
           "addDependency failure");
 ...
 * \note This function does not destroy the dependecy matrix and so can be
 * called multiple times if for instance more dependencies are added and the
 * order needs updating. \pre All relevant objects must have called
 * addDependency() to add their dependencies to the matrix. \sa getOrdering
 */
void DependencyFinder::createOrdering() {
  // If there is an existing stored ordering, clear it.
  mOrdering.clear();

  // Create a vector which marks the objects that are cleared (not a dependent
  // of any non-cleared objects).
  vector<bool> cleared(mDependencyMatrix.size());

  // Search until the ordering contains all objects in the matrix. Or in other
  // words until all objects are cleared.
  while (mOrdering.size() < mDependencyMatrix.size()) {
    ...
    for (size_t objIndex = 0; objIndex < mDependencyMatrix.size() &&
                              objIndexToClear == numeric_limits<size_t>::max();
         ++objIndex) {
      ...
      if (!cleared[objIndex]) {
        ...
        for (size_t depIndex = 0;
             depIndex < mDependencyMatrix.size() && !depFound; ++depIndex) {
          // Check if there is a dependency at this location by and object that
        ...
        // If we did not find a dependency on the current object so then we can
        // now clear it then move on.
        if (!depFound) {
 ...
 */
void DependencyFinder::removeDependency(const size_t aObject,
                                        const size_t aDependency) {
  // Remove the dependency, or edge.
  H_ASSERT(mDependencyMatrix.size() > aObject, "removeDependency failure");
  H_ASSERT(mDependencyMatrix[aObject].size() > aDependency,
           "removeDependency failure");
 ...
 */
DependencyFinder::ObjectIndexMap::iterator
DependencyFinder::addTrackedItem(const string &aItem) {
  ...
  pair<ObjectIndexMap::iterator, bool> newPositionPair =
      mObjectIndices.insert(make_pair(aItem, newLocation));
 ...
 * \note This function is slow as it performs a reverse map lookup and should
 not be used in sections of code which are called frequently.
...
const string &DependencyFinder::getNameFromIndex(const size_t aIndex) const {
  ...
  for (ObjectIndexMap::const_iterator item = mObjectIndices.begin();
       item != mObjectIndices.end(); ++item) {
