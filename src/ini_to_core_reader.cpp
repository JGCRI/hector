 ...
 * through the core. \param filename The INI file to be parsed by the core.
 ...
 *  \exception h_exception Any errors generated by the core while trying to
 * setData.
 */
int INIToCoreReader::valueHandler(void *user, const char *section,
  ...
  try {
    if (startBracket != nameStr.end()) {
      ...
      double valueIndex =
          parseTSeriesIndex(startBracket, endBracket, nameStr.end());
 ...
 *
 *  The date index is assumed to be enclosed within square brackets.  Extra
...
double INIToCoreReader::parseTSeriesIndex(const StringIter startBracket,
  ...
  try {
    ...
    H_THROW("Could not convert index to double: " + dateIndexStr +
