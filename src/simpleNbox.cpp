...
void SimpleNbox::setData(const std::string& varName,
  ...
  if (data.isVal) {
    H_LOG(logger, Logger::DEBUG)
        << "Setting " << biome << "." << varNameParsed << "[" << data.date
        << "]=" << data.value_unitval << std::endl;
  } else {
    H_LOG(logger, Logger::DEBUG)
        << "Setting " << biome << "." << varNameParsed << "[" << data.date
 ...
 *
 *  This is called internally throughout the model run and performs sanity
 * checks. For example, the main carbon pools (except earth) should always be
 * positive; partitioning coefficients should not exceed 1; etc.
 */
...
unitval SimpleNbox::sum_map(unitval_stringmap pool) const {
  ...
  for (unitval_stringmap::const_iterator it = pool.begin(); it != pool.end();
       it++)
    sum = sum + it->second;
  return sum;
}
...
double SimpleNbox::sum_map(double_stringmap pool) const {
  H_ASSERT(pool.size(), "can't sum an empty map");
  double sum = 0.0;
  for (double_stringmap::const_iterator it = pool.begin(); it != pool.end();
       it++)
    sum = sum + it->second;
  return sum;
}
...
void SimpleNbox::log_pools(const double t) {
  // Log pool states
  H_LOG(logger, Logger::DEBUG)
      << "---- simpleNbox pool states at t=" << t << " ----" << std::endl;
...
void SimpleNbox::prepareToRun() throw(h_exception) {
  ...
  if ((has_biome(SNBOX_DEFAULT_BIOME)) & (biome_list.size() > 1)) {
    H_THROW(
        "Cannot have both global and biome-specific data! "
  ...
  omodel = dynamic_cast<CarbonCycleModel*>(
      core->getComponentByCapability(D_OCEAN_C));
...
void SimpleNbox::run(const double runToDate) throw(h_exception) {
  ...
  Tgav_record.set(runToDate,
                  core->sendMessage(M_GETDATA, D_GLOBAL_TEMP).value(U_DEGC));
}
 ...
 * solver \param[in] double       Flat array of carbon pools (no units)
 *  \exception h_exception  If ocean model diverges from our pool tracking all
 * ocean C \exception h_exception  If mass is not conserved
 *
...
void SimpleNbox::stashCValues(double t, const double c[]) {
  ...
  H_LOG(logger, Logger::DEBUG)
      ...
      << "  atm = " << c[0] << "  veg = " << c[1] << "  det = " << c[2]
      << "  soil = " << c[3] << "  ocean = " << c[4] << "  earth = " << c[5]
      << std::endl;
  ...
  if (core->inSpinup() || (CO2_constrain.size() && CO2_constrain.exists(t))) {
    ...
    H_LOG(logger, Logger::DEBUG) << t << "- have " << Ca << " want "
                                 << atmppmv.value(U_PPMV_CO2) << std::endl;
    H_LOG(logger, Logger::DEBUG)
        << t << "- have " << atmos_c << " want " << atmos_cpool_to_match
        << "; residual = " << residual << std::endl;

    // Transfer C from atmosphere to deep ocean and update our C and Ca
    // variables
    H_LOG(logger, Logger::DEBUG)
        << "Sending residual of " << residual << " to deep ocean" << std::endl;
...
int SimpleNbox::calcderivs(double t, const double c[], double dcdt[]) const {
  ...
  //    H_ASSERT( yearfraction >= 0 && yearfraction <= 1, "yearfraction out of
  //    bounds" );
  ...
  for (auto it = biome_list.begin(); it != biome_list.end(); it++) {
    ...
    detsoil_flux = detsoil_flux +
                   unitval(detritus_c.at(biome).value(U_PGC) * 0.6, U_PGC_YR);
  }
  ...
  dcdt[SNBOX_ATMOS] =  // change in atmosphere pool
      ffi_flux_current.value(U_PGC_YR) + luc_current.value(U_PGC_YR) +
      ch4ox_current.value(U_PGC_YR) - atmosocean_flux.value(U_PGC_YR) -
  ...
  /*    printf( "%6.3f%8.3f%8.2f%8.2f%8.2f%8.2f%8.2f\n", t, dcdt[ SNBOX_ATMOS ],
              dcdt[ SNBOX_VEG ], dcdt[ SNBOX_DET ], dcdt[ SNBOX_SOIL ], dcdt[
     SNBOX_OCEAN ], dcdt[ SNBOX_EARTH ] );
  */
  return omodel_err;
}
...
void SimpleNbox::slowparameval(double t, const double c[]) {
  ...
  for (auto it = biome_list.begin(); it != biome_list.end(); it++) {
    ...
    H_LOG(logger, Logger::DEBUG) << "co2fert[ " << biome << " ] at " << Ca
  ...
  // Q10 values The soil pool uses a lagged Tgav, i.e. we assume it takes time
  // for heat to diffuse into soil
  ...
  for (auto it = biome_list.begin(); it != biome_list.end(); it++) {
    ...
    if (in_spinup) {
      ...
      tempfertd[biome] = pow(q10_rh.at(biome),
                             (Tgav_biome / 10.0));  // detritus warms with air
...
#define Q10_TEMPN 200  // 25
      ...
      H_LOG(logger, Logger::DEBUG)
          << biome << " Tgav=" << Tgav << ", Tgav_biome=" << Tgav_biome
