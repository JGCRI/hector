...
//------------------------------------------------------------------------------
// documentation is inherited
void slrComponent::setData(const string &varName, const message_data &data) {
...
void slrComponent::compute_slr(const double date) {
  ...
  unitval T =
      tgav.get(date) - refperiod_tgav; // temperature relative to 1951-1980 mean
  ...
  // temperature." PNAS 106(51): 21527-21532. This incorporates both thermal
  ...
  H_LOG(logger, Logger::DEBUG) << date << " T=" << T << " dTdt=" << dTdt_double
                               << " dHdt=" << dHdt << " slr=" << X << std::endl;
  ...
  unitval dHdt_no_ice;
  dHdt_no_ice.set(a_no_ice * (T - T0_no_ice).value(U_DEGC) +
                      b_no_ice * dTdt_double,
                  U_CM_YR); // cm/yr
  ...
  H_LOG(logger, Logger::DEBUG)
      << date << " T=" << T << " Dtdt=" << dTdt_double
      << " DHdt_no_ice=" << dHdt_no_ice << " slr_no_ice=" << Y << std::endl;
}
...
void slrComponent::run(const double runToDate) {
  ...
  // Sea level rise is different from some of the other model outputs, because
  // the formula used here to compute it depends on knowing a reference period
  // temperature

  tgav.set(runToDate, core->sendMessage(
                          M_GETDATA, D_GLOBAL_TAS)); // store global temperature

  if (runToDate ==
      refperiod_high) { // then compute reference period temperature
    H_LOG(logger, Logger::DEBUG)
        << "Computing reference temperature" << std::endl;
    double sum = 0.0;
    ...
    H_LOG(logger, Logger::DEBUG)
        << "Computed reference temperature " << refperiod_tgav.value(U_DEGC)
...
//------------------------------------------------------------------------------
// documentation is inherited
unitval slrComponent::getData(const std::string &varName, const double date) {
...
//------------------------------------------------------------------------------
// documentation is inherited
void slrComponent::accept(AVisitor *visitor) { visitor->visit(this); }
