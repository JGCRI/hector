...
/*!
 * \brief This class calculates an ordering of named objects through a
 * scheduling algorithm based on supplied dependencies. \details This class can
 * order any type of object that has a name represented by a string. To use this
 * class to create a correct object ordering, the user should first use the
 * addDependency function for each dependency of a object. Once all dependencies
 * have been added, calling createOrdering constructs an internal ordering.
 * Currently cycles will generate and exception.  Additional dependencies can be
 * added once the ordering is created, but createOrdering must be called again.
 * Once createOrdering has completed, the user can then call getOrdering to
 *          return an ordered list of objects, starting with the object which
 * has no dependencies. The two key data structures are mDependencyMatrix and
 * mObjectIndices. mDependencyMatrix is a two-dimensional matrix where each cell
