...
class SimpleNbox : public CarbonCycleModel {
 ...
 public:
  ...
  virtual unitval sendMessage(
      const std::string& message, const std::string& datum,
 ...
 private:
  ...
  tseries<double> Tgav_record;  //!< Record of global temperature values, for
                                //!< computing soil RH
  ...
  tseries<unitval>
      atmosland_flux_ts;  //!< Atmosphere -> land C flux (time series)
  ...
  double calc_co2fert(std::string biome, double time = Core::undefinedIndex())
      const;  //!< calculates co2fertilization factor.
  unitval npp(std::string biome, double time = Core::undefinedIndex())
      const;  //!< calculates NPP for a biome
  unitval sum_npp(double time = Core::undefinedIndex())
      const;  //!< calculates NPP, global total
  unitval rh_fda(std::string biome)
      const;  //!< calculates current RH from detritus for a biome
  unitval rh_fsa(std::string biome)
      const;  //!< calculates current RH from soil for a biome
   ...
   *****************************************************************/
  void sanitychecks() throw(
      h_exception);  //!< performs mass-balance and other checks
  unitval sum_map(unitval_stringmap pool)
