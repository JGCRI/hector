...
/*
 *  message_data.h - A simple struct to hold data to be passed around with
 * messages. hector
 *
...
struct message_data {
  ...
  message_data(const std::string &value)
      : date(Core::undefinedIndex()), value_str(value), isVal(false) {}

  // Create a message data to pass a unitval.
  message_data(const unitval &value)
      : date(Core::undefinedIndex()), value_unitval(value), isVal(true) {}

  // create a message data to pass unitval and date
  message_data(double d, const unitval &val)
      : date(d), value_unitval(val), isVal(true) {}
   ...
   *  \exception h_exception An exception may be raised for the following
   * reasons:
   *      - The unitval units do not match the expected units.
   *      - The value string could not be parsed.
   */
  unitval getUnitval(const unit_types &expectedUnits,
                     const bool strict = false) const {
    if (isVal) {
      if (strict) {
        H_ASSERT(
            value_unitval.units() == expectedUnits,
            "Units: " + value_unitval.unitsName() +
                " do not match expected: " + unitval::unitsName(expectedUnits));
      }
      unitval result =
          value_unitval; // copy to not change value_unitval of message
      result.expecting_unit(expectedUnits);
      return result;
    } else {
      if (strict && units_str.empty() && expectedUnits != U_UNDEFINED) {
        H_THROW("Units: '' do not match expected: " +
                unitval::unitsName(expectedUnits));
      }
