...
double seval_forsythe(int, double, double *, double *, double *, double *,
                      double *);
double seval_deriv_forsythe(int, double, double *, double *, double *, double *,
                            double *);
...
inline void h_interpolator::locate(double x, int &iprev, int &inext) const {
  ...
  /* Search for the data points with independent values containing the
   argument u. */
  if (ilast >= ndata - 1 || ilast < 0)
    ilast = 0;

  /* If u is not in the current in || ilast < 0terval, then execute a binary
   * search. */
