...
$\ GMST = (\text{land air temperature} * \text{fractional land area}) + (\text{ocean surface temperature} * (1 - \text{fractional land area})) $

In Hector, land air temperature is represented by `Tgav_land` (`LAND_AIR_TEMP()`) and ocean surface temperature is represented by `Tgav_ocean_ST` (`OCEAN_SURFACE_TEMP()`). These components stem from the variable `Tgav` (`GLOBAL_TEMP()`), which is how Hector stores GMAT.

Note that the reason that the same variable is represented in two different ways is due to the interaction between the R interface and the underlying C++ model. For example, `LAND_AIR_TEMP()` is the function in R that returns a string message such as `Tgav_land`, which is then passed into the base C++ Hector model. This method is user-friendly as spelling/capitalization is not an issue and developer-friendly as string in C++ can be changed without changing the R user interface.

``` {r find.GMST}
flnd = 0.29;     # fractional land area, a constant in Hector

variables <- c(LAND_AIR_TEMP(), OCEAN_SURFACE_TEMP(), GLOBAL_TEMP())
hector_output <- fetchvars(core, 1745:2300, variables)
...
# Conversion equation
output$GMST <- with(output, (Tgav_land * flnd + Tgav_ocean_ST * (1-flnd)))
