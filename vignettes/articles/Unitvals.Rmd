...
The internal C++ type is called `unitval`, and its code is located in `inst/include/unitval.hpp`. (There is also a restricted type of `unitval` called a `fluxpool` that allows only non-negative values.)
