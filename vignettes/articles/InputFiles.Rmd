...
The primary input file is a text file with *sections*, *properties*, and *values*. For example, the beginning of the [SSP245 file](/inst/input/hector_ssp245.ini) included in the repository looks like this:
...
input <- read.csv("input_params.csv")
...
In this case, the `input/emissions/ssp245_emissions.csv` file is parsed and the data in column `lucEmissions` are assigned to the model variable, with a required `Date` column providing the time index.
