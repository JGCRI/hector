...
This approach requires little to no code experience but does require that the users have radiative forcing values readily available. Users can read in the new radiative forcing values via the ini file with the `Fmisc` variable. `Fmisc` is a column in the hector csv input tables named something like `ssp434_emiss-constraints_rf.csv`. Depending on the user configuration, `Fmisc` may have numeric values or will be equal to all zeros. If `Fmisc` is non-zero, then the user should add the new values to the existing values. Alternatively, using the Hector R package, users can read in miscellaneous forcing to a Hector core with `RF_MISC()` and the `setvar()` function. 
