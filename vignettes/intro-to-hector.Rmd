...
vignette: >
  %\VignetteIndexEntry{A tour of the Hector R interface}
...
If you do not yet have Hector installed, please reference the manual's instructions for [building and running Hector](BuildHector.html). 
...
First, determine the path of the input file (ini) corresponding to the scenario SPP245.
...
Alternatively, users may provide a path to an ini file external to the Hector package on their local machine. This ini file must comply with the Hector  [ini requirements](InputFiles.html). 
...
This means that it is not currently possible to change Hector _parameters_ (such as $\beta$ or preindustrial CO~2~) part-way through a run.
However, it is still possible to change the values of specific _drivers_ and _state variables_ (such as the CO~2~ emissions) in the middle of a run.
...
As expected, increasing CO$_2$ fertilization increases the strength of the terrestrial carbon sink and therefore reduces atmospheric CO$_2$, radiative forcing, and global temperature.
However, the effects only become pronounced in the latter half of the 21st Century.


# Sensitivity analysis

Hector runs quickly, making it easy to run lots of simulations under slightly different configurations.
...
#'   `2000:2300`)
#' @param ini_file Hector parameter file (default = SSP245 file)
