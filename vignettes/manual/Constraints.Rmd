...
then the model's atmospheric CO2 concentration ([CO2], given in ppmv CO2) will follow this contents of this file (which should have two columns, `Date` and `CO2_constrain`). If the constraint [CO2] is lower than the calculated [CO2] at that time step, any excess is moved to the deep ocean; conversely, any necessary deficit is transferred from there.
Otherwise Hector's atmospheric CO2 concentration constraint can be set using the `setvars` function and `CO2_CONSTRAIN` variable.
...
This means that discontinuous constraints can abruptly force Hector's climate system into state that is very inconsistent with the previous time step.
