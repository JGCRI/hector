...
then the model's atmospheric carbon pool (given in ppmv CO2) will follow this contents of this file (which should have two columns, `Date` and `Ca_constrain`). If the constraint [CO2] is lower than the calculated [CO2] at that time step, any excess is moved to the deep ocean; conversely, any necessary deficit is transferred from there.
Otherwise Hector's atmospheric carbon pool can be set using the `setvars` function and `CA_CONSTRAIN` variable.
...
This means that "hybrid" runs are possible, where only some specific time ranges have CO2 constraints while others calculate the atmospheric CO2 according to Hector's standard [carbon cycle](SimpleNBox).
