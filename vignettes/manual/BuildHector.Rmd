...
On **Windows**, you will also need to install Rtools. Download and instructions can be found [here](https://cloud.r-project.org/bin/windows/Rtools/). After installation, there is another step to put the Rtools `make utilities` on the system PATH. This will allow you to compile R packages.
...
Note, if your C++ compiler or standard library does [not support the C++ 17 standard](#no-stdfilesystem-support) then the Boost `system` and `filesystem` libraries will need to be compiled/installed.
...
* You will need to set several custom paths (Preferences -> Locations -> Custom Paths): `BOOSTROOT`, and `HECTORDIR`.
    ...
    * `HECTORDIR` should be set to root directory for the model (the repository root) Xcode will use this path to the ini files used by the various schemes and to write the csv files to the output directory. 
