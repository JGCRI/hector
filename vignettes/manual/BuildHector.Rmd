...
* You will need to set several custom paths (Preferences -> Locations -> Custom Paths): `BOOSTROOT`, and `HECTORDIR`. These are the the Boost root under which the headers can be found in `boost/`, and the runtime working directory for the model (the repository root).
