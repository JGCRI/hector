...
The total C flux from atmosphere to land at time $t$ ($F_{L}(t)$) is the net of C sequestration by photosynthesis (net primary productivity, $NPP_i(t)$) and C release by heterotrophic respiration ($RH_i(t)$) and land use change ($LUC_i(t)$), summed across each biome $i$:

$$ F_{L}(t) = \sum_{i=1}^{n} \left( NPP_i(t) - RH_i(t) - LUC_i(t) \right) $$
