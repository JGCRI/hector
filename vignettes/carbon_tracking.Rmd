...
The `tdata` dataset contains information on year, pool name, amount of units in each pool, and the fraction of each source within that pool. 
...
Now, if we want to examine just one pool, we can do the following. Note that this example requires the `dplyr` package. 
