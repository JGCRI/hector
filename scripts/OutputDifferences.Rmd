...
<!-- Code to pull V3.2.0 data from Zenodo -->

#### Comparison with Previous Release
``` {r differences-3-2-0, echo = FALSE, message = FALSE, include = FALSE}
# Load packages
library(dplyr)
library(ggplot2)
library(hector)
...
```{r, check-any-change-3-2-0, echo=FALSE, results='asis'}
# If there are no differences in outputs, do nothing
# Otherwise, run summary table and graph code
zeros <- rep(0, length(compdata$diff))
SAME <- isTRUE(all.equal(zeros, compdata$diff, tolerance = 1e-7))
if (SAME) {
    msg <- paste0("The current pull request's outputs do not differ from ", 
                  version, " (", commit, ").", sep = "")
    ggsave("same_plot2.png", plot = ggplot(), height = 6, width = 9)
    cat(msg)
} else {
    cat("The current pull request's outputs differ from ", 
        version, " (", commit, ") as follows:", sep = "")  
...
```{r summary-info-3-2-0, echo = FALSE, message = FALSE}
if(!SAME) {
    # Organize data
    output %>%
        select(latest_version = value) -> new_values
...
if(!SAME) {
    ...
    # it a CML report."  https://github.com/iterative/cml#cml-reports
    # So instead of printing the graph, we save it to a file
    ggsave("diff_plot2.png", plot = diff_plot, height = 6, width = 9)
    ...
    ggsave("comp_plot2.png", plot = comp_plot, height = 6, width = 9)
}
```
