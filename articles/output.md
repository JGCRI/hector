...
Output from the standalone executable is written out to `output`
sub-directory of the hector’s root directory. The Hector results are
stored in `outputstream_name.csv`; if applicable, carbon tracking
results are written to `tracking_name.csv` (where the name refers to the
hector run name).

The output stream csv contains results and output from the spin-up
period for a handful of variables.

## R Hector

The [fetchvars](https://jgcri.github.io/hector/reference/fetchvars.md)
function is used to get results from a hector core with the identifier
functions listed as `R_function` in the table below.
